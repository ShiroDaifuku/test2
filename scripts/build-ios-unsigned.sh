...
if [ -n "$IPA" ]; then
  ...
  # 去除 app bundle 里与 data.7z 重复的播种资源（.official + 散装 third_party）。
  # 移动端首启播种只解压 data.7z 到 Documents，bundle 内这些散装文件运行时不会被读取。
  echo "[build-ios] stripping duplicated seed resources from app bundle (.official + loose third_party)..."
  BEFORE_SIZE="$(stat -f%z "$IPA" 2>/dev/null || echo 0)"
  zip -d "$IPA" 'Payload/LingChat.app/assets/data/.official/*' 'Payload/LingChat.app/assets/data/third_party/*' >/dev/null 2>&1 || true
  zip -d "$IPA" 'Payload/LingChat.app/assets/data/.official/' 'Payload/LingChat.app/assets/data/third_party/' >/dev/null 2>&1 || true
  AFTER_SIZE="$(stat -f%z "$IPA" 2>/dev/null || echo 0)"
  echo "[build-ios] IPA size: ${BEFORE_SIZE} -> ${AFTER_SIZE} bytes (saved $((BEFORE_SIZE - AFTER_SIZE)))"
