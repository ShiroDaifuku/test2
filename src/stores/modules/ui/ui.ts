...
interface UIState {
  ...
  // 音乐列表版本号：上传/删除音乐后自增，通知 MusicPlayer 刷新列表
  musicListVersion: number;
}
...
export const useUIStore = defineStore("ui", {
  ...
  actions: {
    ...
    // 音乐列表变更通知（上传/删除后调用，驱动 MusicPlayer 刷新列表）
    bumpMusicListVersion() {
      this.musicListVersion += 1;
    },
    ...
    /**
     * 处理背景音乐结束事件（由 GameBackground 的曲终回调统一触发）
     * 单曲循环：由 AudioAcrossFade 的原生 loop 接管，这里直接返回；
     * 列表循环 / 随机：从服务端取曲目列表，按模式选下一首并写回 currentBackgroundMusic，
     * GameBackground 的 src watch 会自动交叉淡入。
     */
    async handleBackgroundMusicEnd() {
      if (this.bgMusicMode === "loop-single") return; // 原生 loop 已处理
      if (!this.currentBackgroundMusic || this.currentBackgroundMusic === "None") return;

      let tracks: Array<{ name: string; url: string }> = [];
      try {
        tracks = await musicGetAll();
      } catch (e) {
        console.warn("曲终自动切歌失败（拉取列表出错）:", e);
        return;
      }
      if (tracks.length === 0) return;

      const currentUrl = this.currentBackgroundMusic;
      const idx = tracks.findIndex((m) => m.url === currentUrl);
      ...
      if (this.bgMusicMode === "random") {
        if (tracks.length > 1) {
          let pick: string;
          do {
            pick = tracks[Math.floor(Math.random() * tracks.length)]!.url;
          } while (pick === currentUrl);
          nextUrl = pick;
        } else {
          nextUrl = tracks[0]!.url;
        }
      } else {
        nextUrl = tracks[idx !== -1 ? (idx + 1) % tracks.length : 0]!.url;
      }
      if (!nextUrl) return;

      if (nextUrl === currentUrl) {
        // 仅一首歌 + 非单曲循环：引擎的 ended 已经发生，同 URL 不会触发 src watch，
        // 通过一次暂停-恢复让引擎从开头重播该曲。
        this.bgMusicPaused = true;
        this.bgMusicPaused = false;
        return;
      }
      this.currentBackgroundMusic = nextUrl;
      this.bgMusicPaused = false;
      this.bgMusicStoped = false;
    },
