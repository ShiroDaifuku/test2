...
export function useLanSync() {
  ...
  /** 手动添加对等设备（公网隧道地址 + 配对 token） */
  async function addManualPeer(url: string, token: string): Promise<PeerInfo> {
    const peer = await invoke<PeerInfo>("lan_sync_add_manual_peer", { url, token });
    peers.value = [peer, ...peers.value.filter((p) => p.deviceId !== peer.deviceId)];
    selectedPeer.value = peer;
    return peer;
  }

  /** 拉取同步诊断信息（本机 IP、服务状态、端口、已发现设备） */
  async function refreshDebugInfo(): Promise<void> {
    try {
      debugInfo.value = (await invoke("lan_sync_debug_info")) as Record<string, unknown>;
    } catch (e) {
  ...
  function openDialog() {
    ...
    reset();
    startServer().then(() => {
      scanPeers().finally(() => refreshDebugInfo());
    });
  }
