...
export const EMOTION_CONFIG_EMO: EmotionMap = {
  ...
  惊讶: "惊讶",
  正常: "正常",
  // 分类器「平静」在立绘目录里没有同名文件（默认角色只有「正常」），
  // 直接映射到「正常」，避免依赖后端 fallback、且与展示标签一致。
  平静: "正常",
  // ===== DS娘 v0.4 修的别名（上游提示词白名单用的是这几个词，但这里没有对应键，
  //       导致模型输出【无语】【尴尬】【情动】时立绘直接回退成「正常」）=====
  无语: "无奈",
  尴尬: "难为情",
  情动: "心动",
  // 后端 normalize_emotion_tag 会把「难过/悲伤/流泪/伤心/失落/沮丧/委屈/郁闷/低落」
  // 全部归一成「伤心」，而这里原本只有「哭泣」键 → 整个"伤心"家族都会回退成「正常」。
  伤心: "伤心",
  // 我们的自训练小模型直接输出立绘名「羞耻」（而不是标签「难为情」），补一个键兜住
  羞耻: "羞耻",
};
