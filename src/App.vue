...
<script setup lang="ts">
  ...
  onMounted(async () => {
    ...
    if (isMobile()) {
      // 标记移动端：供 CSS 给根元素补深色底（透明桌宠仅桌面端使用）
...
<style>
  ...
  /* 移动端兜底：根元素给深色底，避免 iOS dvh 视口计算异常时底部露出系统黑条。
     透明背景仅桌面端桌宠窗口需要（透明置顶 + hit-test），移动端无此需求。 */
  html.mobile-platform,
  html.mobile-platform body {
    background: #0b0f14;
  }
