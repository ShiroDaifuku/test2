...
<script setup lang="ts">
  ...
  // 把当前台词/动作文本按公式渲染写回显示区（打字完成后调用）
  function applyDialogueMath() {
    const line = currentDisplayedText.value || uiStore.showCharacterLine;
    if (line && dialogueLineRef.value) dialogueLineRef.value.innerHTML = renderMathText(line);
    if (settingsStore.text.inlineMotionText) {
      const motion = uiStore.showCharacterMotionText;
      if (motion && motionLineRef.value) motionLineRef.value.innerHTML = renderMathText(motion);
    }
  }
  ...
  // 移动端：点击屏幕恢复被关闭的对话框（桌面端有右键/Space，移动端没有）
  const handleMobileTapToRestore = (e: Event) => {
  ...
  // 数学公式渲染用防抖：台词分句合并时打字机会短暂停一下再续打，
  // 若每停一次就立即渲染，续打的下一句会覆盖/打断已渲染的公式。
  let mathRenderTimer: ReturnType<typeof setTimeout> | null = null;
  watch(isTyping, (t) => {
