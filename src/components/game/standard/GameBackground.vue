...
<script setup lang="ts">
  ...
  // 曲终统一在这里推进（GameBackground 常驻聊天场景，是唯一可靠的曲终监听点）：
  // 单曲循环由 AudioAcrossFade 原生 loop 处理（不派发 ended）；
  // 列表循环/随机由 ui store 按模式选下一首并写回 currentBackgroundMusic，本组件 src watch 自动淡入。
  const handleTrackEnd = (): void => {
    void uiStore.handleBackgroundMusicEnd();
  };
