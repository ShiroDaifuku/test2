        ...
        <div class="flex-1 space-y-4 overflow-y-auto p-5">
          ...
          <div v-if="view === 'device-list' && phase !== 'scanning'" class="space-y-3">
            ...
            <div class="space-y-2 rounded-2xl border border-white/10 bg-white/5 p-3">
              <p class="text-xs font-semibold text-white/60">手动连接（公网隧道）</p>
              <input
                v-model="manualUrl"
                placeholder="https://xxx.trycloudflare.com"
                class="w-full rounded-lg bg-black/30 px-3 py-2 text-xs text-white outline-none"
              />
              <input
                v-model="manualToken"
              ...
              />
              <button
                @click="emit('manual-add', manualUrl, manualToken)"
                class="w-full rounded-lg bg-cyan-500/80 px-3 py-1.5 text-xs font-semibold text-white
            ...
            <pre
              v-if="debugInfo"
              class="max-h-40 overflow-auto rounded-xl bg-black/40 p-3 text-[10px] leading-relaxed text-white/60"
...
<script setup lang="ts">
  ...
  const emit = defineEmits<{
    ...
    pull: [peer: PeerInfo];
    push: [peer: PeerInfo];
    "manual-add": [url: string, token: string];
    confirm: [];
    cancel: [];
    close: [];
    restart: [];
