...
<script setup lang="ts">
  ...
  // 自动切歌/循环统一由 GameBackground → uiStore.handleBackgroundMusicEnd 处理，
  // 本页不再监听 _musicEndTime 推进（避免设置页打开时与主场景重复切歌）。
  ...
  // 这里不再重复监听（本页背景播放器元素在重构后不再承载真实播放）。
  ...
  const uploadMusic = async () => {
    ...
    try {
      ...
      selectedPaths.value = [];
      await loadMusicList();
      // 通知左下角 MusicPlayer 刷新列表
      uiStore.bumpMusicListVersion();
    } catch (error: any) {
