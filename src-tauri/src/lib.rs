...
/// iOS：配置 AVAudioSession 为 Playback + MixWithOthers，
/// 让 AI 语音、音效与背景音乐混音播放，避免互相打断。
#[cfg(target_os = "ios")]
fn configure_audio_session() {
    ...
    unsafe {
        // 伴音模式（Ambient）：与其它 App 混音、不抢占系统「正在播放」、不打断、
        // 不进后台、不上控制中心/锁屏，适合聊天助手的 BGM/语音/音效。
        // 不再启动即 setActive(true)：避免接管系统音频会话。
        let session: Retained<AnyObject> = msg_send![class!(AVAudioSession), sharedInstance];
        let category = ns_string!("AVAudioSessionCategoryAmbient");
        ...
        tracing::info!(
            "[audio] AVAudioSession setCategory(Ambient) 成功={}",
            error.is_null()
        );
    }
}
...
pub fn run() {
    ...
    // iOS：配置音频会话为混音（AI 语音/音效不打断背景音乐）。放在日志初始化之后，便于记录结果。
    #[cfg(target_os = "ios")]
    ...
    // 移动端额外插件：设 WKWebView.scrollView.contentInsetAdjustmentBehavior = .never，
    // 修复 iOS 上自动安全区 inset 把内容上推、底部露出系统黑条的已知问题（Android 为 no-op）。
    ...
    builder
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            lan_sync::lan_sync_start_server_with_token,
            lan_sync::lan_sync_add_manual_peer,
            lan_sync::lan_sync_debug_info,
            // ── 投屏（Screen Cast）──
