...
use axum::{
    Router,
    body::Body,
    extract::{DefaultBodyLimit, Query, Request, State, State as AxumState},
    http::StatusCode,
    middleware,
...
struct ServerState {
    device_id: String,
    data_dir: PathBuf,
    /// 可选配对 token；设置后所有接口要求 `X-LingChat-Token` 头。
    token: Option<String>,
}
...
/// `token` 非空时要求所有请求携带 `X-LingChat-Token` 头（用于公网隧道场景）。
...
/// token 鉴权中间件：设置了 token 时校验 `X-LingChat-Token` 头。
async fn auth_mw(
    ...
    if let Some(expected) = state.token.as_deref() {
        let got = req
            .headers()
            .get("X-LingChat-Token")
            .and_then(|v| v.to_str().ok());
        if got != Some(expected) {
            return Err(AppError(StatusCode::UNAUTHORIZED, "token 校验失败".to_string()));
        }
    }
