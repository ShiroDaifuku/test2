...
/// 组装对端基址：手动连接用 `base_url`，局域网用 `http://host:port`。
fn base_url(peer: &PeerInfo) -> String {
    if let Some(b) = peer.base_url.as_deref() {
...
/// 有 token 时附加 `X-LingChat-Token` 请求头。
fn auth(req: reqwest::RequestBuilder, peer: &PeerInfo) -> reqwest::RequestBuilder {
    if let Some(t) = peer.token.as_deref() {
        req.header("X-LingChat-Token", t)
    } else {
        req
    }
}
...
pub async fn fetch_remote_manifest(peer: &PeerInfo) -> Result<CompleteManifest, String> {
    let url = format!("{}/manifest", base_url(peer));
    info!("请求对端清单: {}", url);

    let client = &*HTTP_CLIENT;

    let response = auth(client.get(&url), peer)
        .send()
        .await
...
pub async fn check_peer_health(peer: &PeerInfo) -> Result<(), String> {
    let url = format!("{}/health", base_url(peer));
    let client = &*HTTP_CLIENT;
...
pub async fn download_file(
    ...
    let url = format!("{}/file?path={}", base_url(peer), urlencoding(remote_path));
    let client = &*HTTP_CLIENT;

    let response = auth(client.get(&url), peer)
        .send()
        .await
        .map_err(|e| format!("下载文件失败 [{}]: {e}", remote_path))?;

    if !response.status().is_success() {
        return Err(format!("对端返回错误 [{}]: {}", remote_path, response.status()));
    }
    ...
    let mut dest = tokio::fs::File::create(&tmp)
        .await
        .map_err(|e| format!("创建临时文件失败: {e}"))?;

    let mut stream = response.bytes_stream();
    while let Some(chunk) = stream.next().await {
        let chunk = chunk.map_err(|e| format!("下载数据失败 [{}]: {e}", remote_path))?;
        AsyncWriteExt::write_all(&mut dest, &chunk)
            .await
            .map_err(|e| format!("写入文件失败: {e}"))?;
    }
    AsyncWriteExt::flush(&mut dest)
        .await
        .map_err(|e| format!("flush 失败: {e}"))?;
    drop(dest);

    std::fs::rename(&tmp, dest_path).map_err(|e| format!("重命名文件失败: {e}"))?;
...
pub async fn upload_file(
    ...
    let response = auth(client.post(&url).body(body), peer)
        .send()
        .await
...
pub async fn push_delete(peer: &PeerInfo, remote_path: &str) -> Result<(), String> {
    ...
    let response = auth(client.post(&url), peer)
        .send()
        .await
...
pub async fn fetch_db_records(peer: &PeerInfo) -> Result<super::messages::DbRecords, String> {
    let url = format!("{}/db-records", base_url(peer));
    info!("请求对端数据库记录: {}", url);
...
pub async fn push_db_records(
    ...
    let response = auth(client.post(&url).json(records), peer)
        .send()
        .await
